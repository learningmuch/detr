...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./weights_da',
...
def main(args):
    ...
    # 在训练开始前创建csv文件,后面用来放我们需要的东西
    with open(show_file / "loss.csv", mode="w") as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'loss_value'])

    # 在训练开始前创建csv文件,后面用来放我们需要的东西
    with open(show_file / "map.csv", mode="w") as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'map'])
        ...
        if args.output_dir and utils.is_main_process():
            ...
            if coco_evaluator is not None:
                ...
                if "bbox" in coco_evaluator.coco_eval:
                    ...
                    with open(show_file / "map.csv", mode="a") as f:
                        result_info = [f"{i:.4f}" for i in coco_evaluator.coco_eval["bbox"].stats]
                        result_info_str = ' '.join(result_info)
                        map_value = result_info_str.split(' ')[1]
                        writer = csv.writer(f)
                        writer.writerow([epoch, map_value])
